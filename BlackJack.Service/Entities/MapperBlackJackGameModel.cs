...
public static class MapperBlackJackGameModel
{
        ...
        ToModel(this BlackJackGame game, string playerId)
    {
        ...
        foreach (var player in game.Players)
        {
            ...
            {
                var isCurrentPlayer = player.Key.Equals(playerId)
                    || player.Value.Status == PlayerStatusTypes.Complete;
    ...
    private static BlackJackPlayerModel MapPlayer(KeyValuePair<string, IBlackJackPlayer> player, bool showAll)
    {
        return new BlackJackPlayerModel
        {
            Name = player.Value.Name,
            Id = player.Key,
            Hands = MapHand(player.Value.Hands, showAll),
            Status = player.Value.Status
        };
    }

    private static List<HandModel> MapHand(IDictionary<string, Hand> hands, bool showAll)
    {
        List<HandModel> handDtos = new();
        foreach (var hand in hands)
        {
            var cards = MapCards(hand.Value.Cards, showAll);
            var dto = new HandModel
            {
                Cards = cards,
                Identifier = hand.Key,
                Actions = GetHandActions(hand.Value.Actions, showAll),
                CardCount = hand.Value.Cards.Count,
                PointValue = cards.Sum(x => x.Value),
                Status = hand.Value.Status
            };
            handDtos.Add(dto);
        }
        return handDtos;
